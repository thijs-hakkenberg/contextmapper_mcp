...
import { parseCML } from '../src/model/parser.js';
import { serializeCML } from '../src/model/writer.js';
import { validateModel } from '../src/model/validation.js';
  ...
  if (model.contextMap) {
    ...
    for (const rel of model.contextMap.relationships) {
      if (rel.type === 'Partnership' || rel.type === 'SharedKernel') {
        console.log(`    - ${(rel as any).participant1} <-> ${(rel as any).participant2} [${rel.type}]`);
      } else {
        const udRel = rel as any;
        console.log(`    - ${udRel.upstream} -> ${udRel.downstream} [U->D]`);
      }
    }
  }
